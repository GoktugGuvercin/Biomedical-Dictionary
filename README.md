...
### CD8, Most Powerful Opponent of Cancer:

Ligands are a special molecule generated by signaling cells. They are bind to receptors located on the surface of target cells. In that way, these two groups of cells would communicate with each other. At that point, ligand molecules can be considered as a chemical messenger built on protein complexes. When ligand-receptor binding realizes, receptor is triggered and transmit a signal to the cell itself. This results in a change in cell behavior or cell structure. Variation in cell morphology, altering gene transcription or gene translation are just some of possible binding effects, which is also called as cellular affect. 
 
Glycoprotein are important integral-membrane proteins; they are involved in cell-cell interactions. Many receptors in charge of receiving signals from ligands are a type of glycoprotein. CD8 molecule,standing for "cluster of differentiation 8", is a glycoprotein-based co-receptor serving for T-cells. There are many different kinds of receptors, and CD8 co-receptor is categorized under cell-surface receptor. It is dominantly synthesized on cytotoxic T-cells for anti-cancer immune response. Right now, it has an important role on cancer immunotherapies. Tumors express specific antigens, and T-cell receptor (TCR) combined with CD8 can detect and recognize these antigens. In that way, cancer cells can be detected and eradicated by CD8+T cell groups. Proliferation of CD8+T cells in cancer can be detected by semantic segmentation.
